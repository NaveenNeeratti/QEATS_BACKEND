...
public void setUp(){
  ...
  public void  testFindNearbyWithin3km() throws IOException {
    ...
    ArgumentCaptor<Double> servingKmsOffPeakHours = ArgumentCaptor.forClass(Double.class);
    verify(restaurantRepositoryServiceMock,times(1))
    .findAllRestaurantsCloseBy(eq(20.0), eq(30.2), eq(LocalTime.of(3,0)), servingKmsOffPeakHours.capture());
    
    
    ArgumentCaptor<Double> servingKmsPeakHours = ArgumentCaptor.forClass(Double.class);
    verify(restaurantRepositoryServiceMock,times(1))
    .findAllRestaurantsCloseBy(eq(21.0), eq(31.1), eq(LocalTime.of(19,0)), servingKmsPeakHours.capture());

    assertEquals("3.0",servingKmsPeakHours.getValue().toString());
    assertEquals("5.0",servingKmsOffPeakHours.getValue().toString());
  }
